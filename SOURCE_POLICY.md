...
The contract does not authenticate referenced artifacts or prove that described work happened. Readiness means the stored packet is sufficient to continue, not that the task is complete or correct. Applications must disclose these limits beside results and use a fresh deployment when the authoritative source collection or policy changes.
